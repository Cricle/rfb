...
export class RfbClient {
  /**
   * @param {{baseUrl?: string, token?: string|null, timeoutS?: number}} options
  ...
  async #send(method, pathAndQuery, body) {
    ...
    try {
      ...
      throw new TransportError(`forkd request failed: ${reason}`);
    }
    clearTimeout(timer);
  ...
  async waitSnapshot(tag, timeoutS = DEFAULT_WAIT_TIMEOUT_S) {
    validation.sandboxId(tag);
   ...
   */
  async createSandbox(tag, { n = 1, transport = TRANSPORT_NDJSON } = {}) {
    validation.sandboxId(tag);
    ...
    const infos = JSON.parse(this.#expectOk(result));
    return infos.map((info) => new Sandbox(info, this, transport));
  }
  ...
  async connect(sandboxId) {
    ...
    const list = JSON.parse(this.#expectOk(result));
    const info = Array.isArray(list) ? list.find((s) => s.id === sandboxId) : null;
    if (!info) {
      throw new RemoteError('sandbox not found');
    }
    return new Sandbox(info, this, TRANSPORT_NDJSON);
  }

  /** Attach to a sandbox with an explicit transport ("ndjson" | "zbrt"). */
  async connectWithTransport(sandboxId, transport) {
    const sandbox = await this.connect(sandboxId);
    if (transport !== undefined && transport !== null) {
      if (transport !== TRANSPORT_NDJSON && transport !== TRANSPORT_ZBRT) {
        throw new ValidationError(`invalid transport: ${transport}`);
      }
      return new Sandbox(sandbox.info, this, transport);
    }
    return sandbox;
  }

  async pingSandbox(sandboxId) {
    validation.sandboxId(sandboxId);
