...
/** Absolute fs paths are only allowed under the /workspace root (segment boundary). */
function isWorkspacePath(path) {
  return path === '/workspace' || path.startsWith('/workspace/');
}

function badBasePath(path) {
  return (
    typeof path !== 'string' ||
    path.length === 0 ||
...
export function payloadSize(value, max) {
  if (value > max) {
...
export function sandboxId(id) {
  if (typeof id !== 'string' || id.length === 0 || id.length > 128 || !/^[A-Za-z0-9_-]+$/.test(id)) {
...
export function timeoutS(value) {
